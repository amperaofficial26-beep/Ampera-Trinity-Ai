...
[data-testid="stToast"] {{
  ...
  animation: trinityToast7s {_DURASI_MS}ms
             cubic-bezier(.2,.8,.2,1) forwards !important;
}}

/* Sembunyikan ikon bawaan Streamlit. */
[data-testid="stToast"] > div:first-child:not(
...
/* Logo Trinity bergerak dan berputar. */
[data-testid="stToast"]::before {{
  ...
  animation: trinityLogoJadiCentang 2.35s
             cubic-bezier(.34,1.56,.64,1) forwards;
}}

/* Centang muncul setelah logo mengecil. */
[data-testid="stToast"]::after {{
  ...
  box-shadow: 0 5px 14px rgba(105, 70, 37, .30),
              0 0 14px rgba(190,137,75,.35);
  font: 700 21px/1 Arial, sans-serif;
  opacity: 0;
  transform: scale(.2) rotate(-35deg);
  animation: trinityCentangMasuk .55s 2.02s
             cubic-bezier(.34,1.56,.64,1) forwards;
}}

/* Tulisan muncul setelah kotak dan logo. */
...
@keyframes trinityLogoJadiCentang {{
  0% {{
    opacity: 0;
    transform: translateY(13px) scale(.35) rotate(0deg);
  }}
  24% {{
    opacity: 1;
    transform: translateY(-8px) scale(1.12) rotate(0deg);
  }}
  43% {{
  ...
  68% {{
    opacity: 1;
    transform: translateY(-3px) scale(1.02) rotate(360deg);
  }}
  84% {{
    opacity: 1;
  ...
  100% {{
    opacity: 0;
    transform: scale(.12) rotate(405deg);
  }}
}}
