# -*- coding: utf-8 -*-
"""
POPOVER PEMILIH MODEL — tampilan "DNA DOUBLE HELIX" (tema krem Trinity).

Konsep:
  - Bagian atas popover: untai DNA beranimasi — dua untai titik (emas & ungu)
  ...
  - Warna mengikuti tema aplikasi (krem #EDE2D1 / kanvas #E8DCC8 /
    teks ungu gelap #2C1F33 / aksen emas #E8B04B) supaya menyatu.
  - Gaya di-scope pakai :has(.dna-wrap) — popover lain tidak ikut berubah.
...
# ============================================================================
# PENGATURAN — palet disamakan dengan styles.py
# ============================================================================
WARNA_LATAR = "#F2E8D6"      # latar popover = warna kotak input chat
WARNA_PANGGUNG = "#E2D6C1"   # latar area heliks (krem lebih tua)
WARNA_GARIS = "#DBCEB9"      # garis tepi popover & kartu (cokelat pasir)
WARNA_KARTU = "#FFFDF9"      # latar kartu model (putih hangat)
WARNA_AKSEN = "#B45309"      # karamel: glow node aktif & hover (kontras di krem)
...
DNA_CSS = (
    "<style>"
    # ---- popover bertema krem KHUSUS yang berisi .dna-wrap ----------------
    ...
    "box-shadow:0 14px 38px rgba(44,31,51,.16)!important;}"
    # ---- panggung DNA (krem tua, tanpa bintang — biar hangat) -------------
    ...
    f"background:{WARNA_PANGGUNG};"
    f"border:1px solid {WARNA_GARIS};}}"
    ...
    "rgba(180,83,9,.4),rgba(74,53,89,.3));"
    f"animation:dnaBar {PERIODE_S}s ease-in-out infinite;}}"
    # dua basa: emas & ungu, bertukar posisi atas-bawah (fase beda 180 derajat)
    ...
    f".dna-col .da{{background:{WARNA_UNTAI1};"
    "box-shadow:0 0 6px rgba(232,176,75,.75);}"
    f".dna-col .db{{background:{WARNA_UNTAI2};"
    "box-shadow:0 0 5px rgba(74,53,89,.45);}"
    # gelombang: turun-naik + membesar-mengecil (kesan depan/belakang)
    "@keyframes dnaWave{"
    "0%{top:0;transform:scale(1.15);z-index:2;opacity:1;}"
    "50%{top:55px;transform:scale(.72);z-index:1;opacity:.55;}"
    "100%{top:0;transform:scale(1.15);z-index:2;opacity:1;}}"
    "@keyframes dnaBar{0%,100%{opacity:.85;}50%{opacity:.3;}}"
    + _kolom_css() +
    ...
    # tombol: kartu putih hangat, ruang kiri untuk pasangan basa
    ...
    "opacity:.6;z-index:2;transition:all .18s ease;}"
    ...
    "opacity:1;box-shadow:0 0 7px rgba(232,176,75,.85);}"
    "[data-testid='stPopoverBody']:has(.dna-wrap) [class*='model_row_']:hover > div::before{"
    "opacity:1;box-shadow:0 0 7px rgba(74,53,89,.6);}"
