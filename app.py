...
# ============================================================================
# ============================================================
# DIAGNOSTIC PANEL — AMPERA TRINITY AI
# ============================================================

def render_system_diagnostic():
    import os
    import shutil
    import subprocess
    import sys

    st.markdown("## 🖥️ Trinity System Diagnostic")
    st.caption("Informasi environment tempat Ampera Trinity AI sedang berjalan.")
    ...
    try:
        ...
        with c2:
            st.metric(
                "RAM Terpakai",
                f"{vm.used / 1024**3:.2f} GB"
            )

        with c3:
            st.metric(
                "RAM Proses Trinity",
                f"{process_ram / 1024**3:.2f} GB"
            )

    except Exception as e:
        st.warning(f"RAM tidak dapat dicek: {e}")
    ...
    try:
        ...
        st.warning(f"CPU tidak dapat dicek: {e}")
    ...
    try:
        ...
        st.warning(f"Storage tidak dapat dicek: {e}")
    ...
    try:
        ...
        st.write(
            f"**CUDA tersedia:** {'✅ YA' if cuda_available else '❌ TIDAK'}"
        )
        ...
        st.warning(f"GPU tidak dapat dicek: {e}")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            st.error("❌ FFmpeg ditemukan tetapi gagal dijalankan.")

    except FileNotFoundError:
        st.error("❌ FFmpeg belum tersedia.")

    except Exception as e:
        st.warning(f"FFmpeg tidak dapat dicek: {e}")
    ...
    if found:
        st.success(
            "Folder/model terkait ditemukan: "
            + ", ".join(found)
        )
    else:
        st.info(
            "Real-ESRGAN belum ditemukan di project."
        )
