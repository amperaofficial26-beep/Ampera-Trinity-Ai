...
Mode chat khusus dengan persona perencana (JADWAL_PROMPT di config.py),
plus daftar tugas yang tersimpan di session_state:

  - Yuki bisa mengisi daftar otomatis lewat blok [[TUGAS]] pada jawabannya
  - Tugas dikelompokkan jadi agenda (Hari Ini / Besok / Minggu Ini / Nanti)
  - Ada prioritas, tandai selesai, hapus, dan ringkasan kemajuan

Riwayat pesannya terpisah: state.mode_thread("jadwal").
"""
...
# ============================================================================
# >>> ATUR TOMBOL CEPAT & KELOMPOK AGENDA DI SINI <<<
# ============================================================================
TOMBOL_CEPAT = [
    ("Rencana belajar",
...
# Kata kunci "kapan" -> kelompok agenda
KELOMPOK = [
...
# Penanda blok tugas dari jawaban Yuki (dibuat longgar seperti kartu lain)
...
def _render_daftar_tugas() -> None:
    daftar = tasks()
    if not daftar:
        st.markdown(
            '<div class="empty-card">Belum ada tugas. Minta Yuki menyusun '
            "rencana, atau tambah sendiri di kotak bawah.</div>",
            unsafe_allow_html=True,
        )
        return

    selesai = sum(1 for t in daftar if t["selesai"])
    persen = int(selesai / len(daftar) * 100)
    st.markdown(
        f'<div class="jd-progress-wrap">'
        f'<div class="jd-progress-text">{selesai} dari {len(daftar)} tugas selesai</div>'
        f'<div class="jd-progress-bar"><div class="jd-progress-fill" '
        f'style="width:{persen}%"></div></div></div>',
        unsafe_allow_html=True,
    )

    urutan = ["Hari ini", "Besok", "Minggu ini", "Nanti"]
    for nama in urutan:
        anggota = [t for t in daftar if _kelompok_dari(t["kapan"]) == nama]
        if not anggota:
            continue
        st.markdown(f'<div class="jd-group">{html.escape(nama)}</div>',
                    unsafe_allow_html=True)
        for t in anggota:
            with st.container(key=f"jd_row_{t['id']}"):
                c_cek, c_isi, c_hapus = st.columns([0.09, 1.0, 0.09])
                with c_cek:
                    st.button(
                        "✓" if t["selesai"] else "○",
                        key=f"jd_tog_{t['id']}",
                        help="Tandai selesai",
                        on_click=toggle_task, args=(t["id"],),
                    )
                with c_isi:
                    ...
                    st.markdown(
                        f'<div class="jd-item {coret}">'
                        f'<span class="jd-dot" style="background:{warna}"></span>'
                        f'<span class="jd-title">{html.escape(t["judul"])}</span>'
                        f'<span class="jd-meta">{" · ".join(meta)}</span>'
                        f"</div>",
                        unsafe_allow_html=True,
                    )
                with c_hapus:
...
def page_jadwal() -> None:
    ...
    st.markdown(
        ...
        '<div><h2 class="page-title">AI Penjadwal</h2>'
        '<p class="page-sub">Pecah tujuan besar jadi langkah kecil, lalu '
        'susun jadi jadwal yang masuk akal.</p></div></div>',
        unsafe_allow_html=True,
    )

    kiri, kanan = st.columns([1.05, 1.0], gap="medium")

    with kiri:
        st.markdown('<div class="jd-panel-title">Daftar tugas</div>',
                    unsafe_allow_html=True)
        _render_daftar_tugas()
    ...
    with kanan:
        st.markdown('<div class="jd-panel-title">Tanya Yuki</div>',
                    unsafe_allow_html=True)
