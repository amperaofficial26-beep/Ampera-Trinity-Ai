...
from ui_helpers import logo_img_html
...
# ----------------------------------------------------------------------------
ACCT_MENU_X_PX = 172   # jarak dari tepi KIRI layar
...
def _export_conversation_text(conv: dict) -> str:
    lines = [
        ...
        f"# Tanggal Ekspor: {__import__('datetime').datetime.now().strftime('%d/%m/%Y %H:%M:%S')}",
        "# by Ampera Official",
        "",
        "---",
        "",
...
def _hapus_percakapan_lokal(conv_id: int) -> None:
    ...
    st.session_state.pop(
        f"_hist_delete_confirm_{conv_id}",
        None,
    )
...
[class*="st-key-hist_menu_"] button {
...
.stApp:has(.tr-chat-layout)
section[data-testid="stSidebar"]
[class*="st-key-hist_menu_"] button:hover {
    background: #E0D2BB !important;
    color: #2C1F33 !important;
}
...
def render_sidebar() -> None:
    with st.sidebar:
        ...
        st.markdown('<div class="sb-divider"></div>', unsafe_allow_html=True)
         ...
         # Riwayat percakapan
        st.markdown(_RIWAYAT_ITEM_CSS, unsafe_allow_html=True)
        ...
        if convs:
            ...
            st.markdown(
                '<div class="sb-group">Hari ini</div>',
                unsafe_allow_html=True,
            )
            ...
            for c in convs[:15]:
                ...
                conv_id = c["id"]
                title = c.get("title") or "Percakapan baru"
                ...
                with col_chat:
                    ...
                    with st.container(
                        key=f"sb_hist_{conv_id}"
                    ):
        
                        if st.button(
                            title,
                            key=f"btn_sb_hist_{conv_id}",
                            use_container_width=True,
                        ):
                    ...
                    with st.container(
                        ...
                        with st.popover(
                            ...
                            nama_baru = st.text_input(
                                "Nama chat",
                                value=title,
                                key=f"hist_rename_input_{conv_id}",
                            ...
                            if not st.session_state.get(confirm_key):
                                ...
                                with d1:
                                    ...
                                    if st.button(
                                        "Batal",
                                        key=f"hist_delete_cancel_{conv_id}",
                                        use_container_width=True,
                                    ):
                                        st.session_state.pop(
                                            confirm_key,
                                ...
                                with d2:
                                    ...
                                    if st.button(
                                        ":material/delete_forever:  Ya, hapus",
                                        key=f"hist_delete_yes_{conv_id}",
                                        type="primary",
                                        use_container_width=True,
                                    ):
                                        _hapus_percakapan_lokal(
                                            conv_id
                                        )
                            ...
                            st.download_button(
                                ...
                                file_name=f"trinity-chat-{conv_id}.md",
                                mime="text/markdown",
                                use_container_width=True,
                                key=f"hist_download_{conv_id}",
                            )
